...
int intSize() {
  ...
  intPtr1 = intArray;    //sets intPtr1 to first address of intArray (&intArray[0])

  intPtr2 = intPtr1;    //sets intPtr2 to &intArray[0]

  intPtr1++;           //sets intPtr1 to &intArray[1]

  return (intPtr1 - intPtr2) *4;  //subtracts the addresses and multiplies by sizeof(int)
}
...
int doubleSize() {
  ...
  doubPtr1 = doubArray;   //sets doubPtr1 to first address of doubArray (&doubArray[0])

  doubPtr2 = doubPtr1;  //sets doubPtr2 &doubArray[1]

  doubPtr1++;  //sets doubPtr1 to &doubArray[1]

  return (doubPtr1 - doubPtr2) *8; //subtracts the addresses and multiplies by sizeof(double)
}
...
int pointerSize() {
  ...
  ptrPtr1 = ptrArray; //sets ptrPtr1 to first address of ptrArray (&ptrArray[0])

  ptrPtr2 = ptrPtr1; //sets ptrPtr2 to &ptrArray[0])

  ptrPtr1++;

  return (ptrPtr1 - ptrPtr2) *8; //subtracts the addresses and multiplies by sizeof(pointer)
}
...
int changeValue() {
  ...
  intPtr2 = (intPtr1 + 5);   //intPtr2 = &intArray[5]

  a += 194;      //sets a = 449

  *intPtr2 = a;  //intArray[5] = 449
...
int withinSameBlock(int *ptr1, int *ptr2) {
  ...
  address1 >>= 6;  //gets top 26 bits
  address2 >>= 6;  //gets top 26 bits

  return address1 == address2;  //checks if top 26 bits are equal
}
...
int withinArray(int * intArray, int size, int * ptr) {
  // Your code here
  int address1 = intArray;
  int address2 = ptr;

  int length = address2 - address1;
  length >>= size;
  size >>= size;   //shifts by size to check if the ptr is in intArray
...
int stringLength(char * s) {
  ...
  while(* s != '\0'){
    length++;         //this adds 1 to length
    s++;              //this moves s to next character
  }
...
void selectionSort(int arr[], int arrLength) {
  int i, j, min_index;
  // Your code here
  int * address; //starting arr address
  address = arr;

  for(i = 0; i < arrLength - 1; i++) {
    min_index = i;
    for(j = i + 1; j < arrLength; j++) {
      if(*(address + min_index) > *(address + j)) {  //if(arr[min_index] > arr[j])
        min_index = j;
      }
    }
    swapInts((address + i), (address + min_index));
  }
